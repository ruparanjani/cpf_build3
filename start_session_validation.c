...
memset(command,0,sizeof(command));
memset(Uid,0,sizeof(Uid));
int i=0,j=0,count=0,sid_category=0,uid=0,iUid=0,check=0;
if(cBuffer[0]!='\n'){
...
if((flag1==true) && (flag2==true) && (flag3==true)){
check=check_sid(Uid,sid);
if(check == 1)
return 1;
else{
printf("\n First you need to subscribe the particular service\n");
return 0;
}
}
else
...
int check_sid(char *sUid,char *sSid){
int i=0,j=0;
int uid,sid;
uid=atoi(sUid);
sid=atoi(sSid);
for(i=0;i<scount;i++){
if(sub[i].uid==uid && sub[i].sid==sid){
j++;
}
}
if(j>0)
