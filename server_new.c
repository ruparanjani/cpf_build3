...
void *client_thread ( void *arg ){               //thread function
	...
	strcpy(buffer, "..........welcome.........");
		...
		if(msg_len<=0){
			...
			if(return_code == 1){
				ibuffer = atoi(resp_output);
				printf("ibuffer => %d ",ibuffer);
				if(ibuffer > 1000 && ibuffer < 3000){
					strcpy(buffer1,"Existing user,your uid is ");
					strcat(buffer1,resp_output);
				}

				else if(ibuffer > 3000 && ibuffer < 4000){
					strcpy(buffer1,"Existing service provider,your sid is ");
					strcat(buffer1,resp_output);
				}
				else
					printf("Neither uid nor Sid");
