...
int subscribe_length_checking(char *cBuffer,int reg_uid){
	...
	if(cBuffer[0]!='\n'){
		...
		if(iUid==reg_uid)
			...
			printf("\n in sub validation Incorrect! ,Please enter your registered UID, re-enter again\n ");
			return 0;
		}
...
void writing_struct(char *uid,char *sid){
strcpy(sub.uid,uid);
strcpy(sub.sid,sid);
}*/
 #include <stdio.h>
...
#define MAX 255
//struct subscribe_details sub1[100];
static int i;
int  subscribe_validation(char *str,int uid)
{
    int i=0,len=0,rv=0;
